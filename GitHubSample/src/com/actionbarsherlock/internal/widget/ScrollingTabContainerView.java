...
public class ScrollingTabContainerView extends NineHorizontalScrollView
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (childCount > 1 &&
                (widthMode == View.MeasureSpec.EXACTLY || widthMode == View.MeasureSpec.AT_MOST)) {
            if (childCount > 2) {
        ...
        heightMeasureSpec = View.MeasureSpec.makeMeasureSpec(mContentHeight, View.MeasureSpec.EXACTLY);

        final boolean canCollapse = !lockedExpanded && mAllowCollapse;

        if (canCollapse) {
            // See if we should expand
            mTabLayout.measure(View.MeasureSpec.UNSPECIFIED, heightMeasureSpec);
            if (mTabLayout.getMeasuredWidth() > View.MeasureSpec.getSize(widthMeasureSpec)) {
                performCollapse();
            } else {
                performExpand();
            }
    ...
    public void addTab(ActionBar.Tab tab, int position, boolean setSelected) {
        ...
        mTabLayout.addView(tabView, position, new IcsLinearLayout.LayoutParams(
                0, FrameLayout.LayoutParams.MATCH_PARENT, 1));
        if (mTabSpinner != null) {
            ((TabAdapter) mTabSpinner.getAdapter()).notifyDataSetChanged();
        }
        if (setSelected) {
